...
// 	var ui = kendo.ui, //TODO: Not working. kendo = undefined 
	...
	var Console = Widget.extend({
        ...
        options: { 
             // the name is what it will appear as off the kendo namespace(i.e. kendo.ui.Console). 
        	 // The jQuery plugin would be jQuery.fn.kendoConsole.   
             name: "Console",
             height: "200px",
             limit: 50
        },
        ...
        _create: function() {
        	...
        	$(that.element).addClass('console');
        },
        
		log: function(message, isError) { 
            var oldContainer = this.element, 
                counter = oldContainer.find(".count"),
                logs = $(oldContainer).children(),
                limit = this.options.limit
                
            //enforce limit   
            if($.isNumeric(limit) && logs.length > limit){
                logs.slice(limit,logs.length).remove();
            }
